...
var import_pangu_ts = __toESM(require("pangu-ts"));
...
function src_default(md, options = {}) {
  const { additionalRules = ["code_inline"] } = options;
  md.renderer.rules.text = (tokens, index, options2, env, self) => {
    const prevChar = getPrevChar(tokens, index);
    return (0, import_utils.escapeHtml)(import_pangu_ts.default.spacing(prevChar + tokens[index].content).slice(prevChar.length));
  };
  additionalRules.forEach((type) => {
    ...
    md.renderer.rules[type] = (tokens, index, options2, env, self) => {
      let output = rule(tokens, index, options2, env, self);
      if (output.length) {
        if (index > 0 && !(0, import_utils.isWhiteSpace)(output.charCodeAt(0))) {
          output = " " + output;
        }
        if (index < tokens.length - 1 && !(0, import_utils.isWhiteSpace)(output.charCodeAt(output.length - 1))) {
          output = output + " ";
        }
      }
      return output;
